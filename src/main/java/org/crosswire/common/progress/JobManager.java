...
/**
 * @brief JobManager is responsible for creating jobs and informing listeners about the
...
public class JobManager 
{
    ...
    private static final String THISMODULE = "JobManager";
    private static final Logger lgr = LoggerFactory.getLogger(JobManager.class);

    private JobManager() {    }

    /**
     * @brief Create a new Job that cannot be canceled.
     *
     * @param jobName the name of the Job
     * @return the job
     */
    public static Progress createJob(String jobName) { return createJob(UUID.randomUUID().toString(), jobName, null); }

    /**
     * @brief Create a new Job that can be canceled.
     *
    ...
    public static Progress createJob(String jobID, String jobName, Thread workerThread) 
    {
        ...
        if ( debug_log ) 
        {
            String msg = "job starting ... " + job.getJobName();
     ...
     */
    public static void addWorkListener(WorkListener li) { listeners.add(li); }
     ...
     */
    public static void removeWorkListener(WorkListener li) { listeners.remove(li); }

    /**
     * @brief Accessor for the currently known jobs
     *
     * @return an iterator over the jobs
     */
    public static Iterator<Progress> iterator() { return jobs.iterator(); }

    /**
     * @return the number of current jobs
     */
    public static int getJobCount() { return jobs.size(); }

    /**
     * @brief Inform the listeners that a title has changed.
     *
    ...
    protected static void fireWorkProgressed(Progress job) {
        ...
        if ( job.isFinished() ) 
        {
            String msg = "job finished: " + job.getJobName();
            if ( debug_log ) { lgr.debug(msg,THISMODULE); }
            jobs.remove(job);
        }
    }
}